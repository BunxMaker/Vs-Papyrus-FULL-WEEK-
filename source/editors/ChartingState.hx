...
class CheatingState extends MusicBeatState
{
	public var dialogue:Array<String> = [
		...
		":text:::*(Ring, ring...)",
		":sans:normal::heya.",
		":sans:comic::is anyone there...?",
		":sans:normal::well ...just asking",
		":sans:closed::i'll be honest",
		":sans:closed::i have no idea what happened for you to get here.",
		":sans:normal::this is actually some sort error-handling message",
		":sans:closed::so, if you are here",
		":sans:bad::you're just a dirty hacker, aren't you?",
		":sans:comic::yeah, get outta here.",
		":sans:wink::do what you are supposed to do.",
		":text:::*(Click...)",
	];
...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Transition Static', "Transition stuff for Triple Skeletons\nValue 1: which static (nothing or gaster)"],
		['Jumpscare Skeletons', "Spooky Skeleton Jumpscare\nValue 1: which skeleton (sans,papyrus,gaster)\nif null will display sans"],
		['Flip Screen', "Sans flips your screen upside down\nValue 1: direction (up or down, default is down)"],
		['Both Sing',"used only in Bone Brothers\nboth skeletons sing at the same time how cool\nValue 1: true or not"],
		['BG Change',"Switches the visibility of 2 BG \n(Only Works on Triple Skeletons)\nValue 1: The BG that will be visible"],
	...
	**/
	...
	override function create()
	{
		...
		menace = new FlxSprite(900, 400);
		menace.frames = Paths.getSparrowAtlas('thefuck', 'shared');
		menace.animation.addByPrefix('dance', 'prus',24);
		menace.scale.set(0.6,0.6);
		menace.updateHitbox();
		menace.scrollFactor.set();
		menace.flipX = true;
		...
		add(zoomTxt);
		if(FlxG.random.bool()) add(menace);
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data] && !note.noSound) {
	...
	function secret() 
	{
		...
		FlxTween.tween(menace, {x: 0, y: 0}, 1, {ease: FlxEase.quadIn});
		FlxTween.tween(menace, {angle: 360}, 1, {ease: FlxEase.quadIn});
	...
	*/
	...
	function loadJson(song:String):Void
	{
		...
		/*switch (format)
			{
				case 'entry-log': if(!FreeplayState.songUnlock[2]) cheattt = true;
				case 'gasterpurgation': if(!FreeplayState.songUnlock[3]) cheattt = true;
				case 'to-the-bone': if(!FreeplayState.songUnlock[3]) cheattt = true;
				case 'triple-skeletons': if(!FreeplayState.songUnlock[3]) cheattt = true;
				case 'final-boss': if(!FreeplayState.songUnlock[6]) cheattt = true;
			}*/
	}
