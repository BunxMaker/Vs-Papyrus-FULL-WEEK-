...
class CharacterEditorState extends MusicBeatState
{
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.skin = parsedJson.noteskin;
	...
	var charNoteSkinInputText:FlxUIInputText;
	var cnsDescrip:FlxText;
	...
	function addCharacterUI() {
		...
		charNoteSkinInputText = new FlxUIInputText(healthIconInputText.x + healthIconInputText.width + 20, healthIconInputText.y, 75,char.skin, 8);
		cnsDescrip = new FlxText(charNoteSkinInputText.x, charNoteSkinInputText.y - 18, 0, 'Char NoteSkin:');
		...
		tab_group.add(charNoteSkinInputText);
		tab_group.add(cnsDescrip);
		...
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == healthIconInputText) {
				...
				if(!Paths.fileExists('images/note stuff/skin/' + charNoteSkinInputText.text + '.png', IMAGE,false,'shared')){
					...
					char.skin = charNoteSkinInputText.text;
					cnsDescrip.text = "Char NoteSkin: FOUND";
				}
			}
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			charNoteSkinInputText.text = char.skin;
	...
	override function update(elapsed:Float)
	{
		...
		var inputTexts:Array<FlxUIInputText> = [animationInputText, imageInputText, healthIconInputText,charNoteSkinInputText , animationNameInputText, animationIndicesInputText];
